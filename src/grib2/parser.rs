...
fn read_u8(data: &[u8], offset: usize) -> Result<u8, String> {
    data.get(offset)
        .copied()
        .ok_or_else(|| format!("read_u8: offset {} out of range (len={})", offset, data.len()))
}
...
fn parse_grid_template_30(sec: &[u8], grid: &mut GridDefinition) -> Result<(), String> {
    ...
    // Dx and Dy are stored in metres as unsigned integers
    grid.dx = read_u32(sec, 55)? as f64;
    grid.dy = read_u32(sec, 59)? as f64;
