...
pub fn find_nearest_site(lat: f64, lon: f64) -> String {
...
pub fn find_latest_file(site: &str, date: &str) -> Option<String> {
...
pub fn http_get_bytes(url: &str) -> Vec<u8> {
    let response = ureq::get(url)
        .header("User-Agent", USER_AGENT)
        .call();
...
pub fn maybe_decompress_gz(data: Vec<u8>) -> Vec<u8> {
