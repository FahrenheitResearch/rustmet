// cmd_history.rs — Observation history with trends
//
// Expected bandwidth: ~2-5KB (multiple METARs from aviationweather.gov)
...
pub fn run(station: &str, hours: u32, pretty: bool) {
    match wx_obs::fetch::fetch_recent_metars(station, hours) {
        Ok(mut metars) => {
            // Sort chronologically (oldest first) by day/hour/minute
            metars.sort_by(|a, b| {
            ...
            // Compute trends: compare average of first 3 vs last 3 observations
            let n = metars.len();
